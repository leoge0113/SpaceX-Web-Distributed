...
public class DistributedReentrantLockExeTempImpl implements DistributedReentrantLockExeTemplete {
    ...
    private CuratorFramework clinet;

    public DistributedReentrantLockExeTempImpl(CuratorFramework clinet) {
        this.clinet = clinet;
    }

    @Override
    public Object execute(String lockId, int timeout, ExecuteCallBack callBack) {
        DistributedLockImpl distributedLock = new DistributedLockImpl(clinet, lockId);
